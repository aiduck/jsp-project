...
export default {
    ...
    methods: {
      initProjectMember(pageNum,pageSize){
            ...
            console.log(params)
            axios.post('/api/jsp/FindAllMemberByStu',qs.stringify(params))
            .then(response=>{
                this.tableData = [];
            ...
            axios.post('/api/jsp/CountOther',qs.stringify(stuId))
            .then(response=>{
                // this.totalPage = response.data.datas;
                response.data.datas.forEach(element=>{
                    if(element.num_name == "学生项目成员"){
      ...
      handleMore(index,row){
          this.$router.push(`/StudentProjectMemberInfo/${row.id}/${row.stu_id}`);
          console.log("更多按钮")
      },
