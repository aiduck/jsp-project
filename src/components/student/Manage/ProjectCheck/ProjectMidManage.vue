<template>
    <div class="wrap">
        ...
        <div class="admin-manage">
            <span style="position:absolute;left:80px">项目管理&nbsp; >&nbsp;项目中期查看</span>
            <el-row>
...
export default {
    ...
    mounted: function() {
        ...
        this.initMidPro(this.currentPage,this.pageSize,this.userId);
        
    },
    methods: {
       initMidPro(pageNum,pageSize,userId){
            let params = {
                pageNum: pageNum,
                pageSize: pageSize,
                userId:this.userId
            }
            axios.post('/api/jsp/FindAllStuMidPro',qs.stringify(params))
            .then(response=>{
                this.tableData = [];
        ...
        handleMore(index,row){
            // console.log(row);
            this.$router.push(`/StudentMidManageInfo/${row.proId}`);
            // this.$router.push({path:'Notifyinfo',query:{title:row.title,type:0}});
            console.log("更多按钮");
        },
        // handleEdit(index,row){
        //     this.$router.push(`/MidManageInfo/1/${row.title}`);
        //     console.log("编辑按钮");
        // },
        // handleDelete(){
        //     console.log("删除按钮");
