<template>
    <div class="wrap">
        ...
        <div class="admin-manage">
            <span style="position:absolute;left:80px">项目管理&nbsp; >&nbsp;项目评审结果查看</span>
            <el-row>
...
export default {
    ...
    methods: {
        initProRev(pageNum,pageSize){
            ...
            console.log(params);
            axios.post('/api/jsp/FindAllStuProReview',qs.stringify(params))
            .then(response=>{
                this.tableData = [];
              ...
              .then(response=>{
                  // console.log(response.data.datas);
                  response.data.datas.forEach(element=>{
                      if(element.num_name == "学生端评审信息"){
                          this.totalPage = element.num;
                        //   console.log("数量"+this.totalPage);
                      }
                  });
              }).catch(err=>{
                  console.log(err);
        ...
        handleMore(index,row){
            this.$router.push(`/StudentEvaluateManageInfo/${row.proId}`);
            console.log("更多按钮");
        },
