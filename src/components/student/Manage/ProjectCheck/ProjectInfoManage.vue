<template>
    <div class="wrap">
        ...
        <div class="admin-manage">
            <span style="position:absolute;left:80px">项目管理&nbsp; >&nbsp;项目信息查看</span>
            <el-row>
...
export default {
    ...
    mounted: function() {
        ...
        this.initProInfo(this.currentPage,this.pageSize,this.userId);
    },
    methods: {
        initProInfo(pageNum,pageSize,userId){
            let params = {
                pageNum: pageNum,
                pageSize: pageSize,
                userId:this.userId
            ...
            axios.post('/api/jsp/FindAllStuProInfo',qs.stringify(params))
            .then(response=>{
                this.tableData = [];
        ...
        handleMore(index,row){
            this.$router.push(`/StudentProjectInfo/${row.proId}`);
            console.log("更多按钮");
        },
        // handleEdit(index,row){
        //     this.$router.push(`/ProjectInfo/1/${row.title}`);
        //     console.log("编辑按钮");
        // },
        // handleDelete(){
