...
export default {
    ...
    mounted:function(){
        ...
        this.initProFile(this.currentPage,this.pageSize);
    },
    methods: {
        initProFile(pageNum,pageSize){
          let params = {
            pageNum: pageNum,
            pageSize: pageSize,
            userId: this.userId
          ...
          console.log(params);
          axios.post('/api/jsp/FindProjectFileByTea',qs.stringify(params))
          .then(response=>{
              this.tableData = [];
              ...
              response.data.datas.forEach(element => {
                this.tableData.push({
                    ...
                    tel: element.leaderObj.user_phone,
                    is_submit:element.proObj.is_submit
                })
              })
          }).catch(err=>{
            console.log(err);
        ...
        handleMore(index,row){
            this.$router.push(`/TeacherMaterialUpload/${row.proId}/${row.title}`);
            console.log("上传按钮");
        },
