...
export default {
    ...
    methods: {
        initProRev(pageNum,pageSize){
            ...
            axios.post('/api/jsp/FindAllProRevByTea',qs.stringify(params))
            .then(response=>{
                this.tableData = [];
              ...
              // console.log(teaId);
              axios.post('/api/jsp/Count',qs.stringify(teaId))
              .then(response=>{
                  // console.log(response.data.datas);
                  response.data.datas.forEach(element=>{
                      if(element.num_name == "教师项目评审结果"){
       ...
       initReviewMsg(index,row){
        ...
        handleMore(index,row){
            this.$router.push(`/TeacherEvaluateManageInfo/${row.proId}`);
            console.log("更多按钮");
        },
