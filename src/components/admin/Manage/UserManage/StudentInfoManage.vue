<template>
 <div class="wrap">
    ...
    <div class="admin-manage">
        <span style="position:absolute;left:80px">用户管理&nbsp; >&nbsp;学生信息管理</span>
        <el-row>
...
import qs from 'qs'
    export default {
        ...
        mounted: function() {
            this.initStudent(this.currentPage,this.pageSize);
        },
        watch:{
            '$route'(to,from){
            this.initStudent(this.currentPage,this.pageSize);
        ...
        methods:{
            initStudent(pageNum,pageSize){
                let params = {
                    // filter: filter, 筛选
                    pageNum: pageNum,
                    pageSize: pageSize
                }
                axios.post('/api/jsp/FindStuAll',qs.stringify(params))
                .then(response=>{
                    this.tableData = [];
            ...
            handleMore(index,row){
                this.$router.push(`/StudentInfo/0/${row.userId}`);
            },
            handleEdit(index,row){
                this.$router.push(`/StudentInfo/1/${row.userId}`);
            },
