<template>
 <div class="wrap">
    ...
    <div class="admin-manage">
        <span style="position:absolute;left:80px">用户管理&nbsp; >&nbsp;教师信息管理</span>
        <el-row>
...
import qs from 'qs'
    export default {
        ...
        mounted: function() {
            this.initTeacher(this.currentPage,this.pageSize);
        },
        watch:{
            '$route'(to,from){
                this.initTeacher(this.currentPage,this.pageSize);
        ...
        methods:{
            initTeacher(pageNum,pageSize){
                let params = {
                    // filter: filter, 筛选
                    pageNum: pageNum,
                    pageSize: pageSize
                }
                axios.post('/api/jsp/FindTeaAll',qs.stringify(params))
                .then(response=>{
                    this.tableData = [];
            ...
            handleMore(index,row){
                this.$router.push(`/TeacherInfo/0/${row.userId}`);
                console.log("更多按钮");
            },
            handleEdit(index,row){
                this.$router.push(`/TeacherInfo/1/${row.userId}`);
                console.log("编辑按钮");
            },
