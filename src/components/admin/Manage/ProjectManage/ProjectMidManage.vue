        ...
        <el-row class="table">
            ...
            <el-table-column
                ...
                <el-button
                    ...
                    v-html="scope.row.protime ==='中期' ? '审核通过' : '审核失败'"></el-button>
                <!-- <el-button
                    size="small"
                    type="danger"
...
export default {
    ...
    methods: {
        initMidPro(pageNum,pageSize){
                ...
                axios.post('/api/jsp/FindAllMidPro',qs.stringify(params))
                .then(response=>{
                    this.tableData = [];
        ...
        handleMore(index,row){
            this.$router.push(`/MidManageInfo/0/${row.proId}`);
            // this.$router.push({path:'Notifyinfo',query:{title:row.title,type:0}});
        },
        handleEdit(index,row){
            this.$router.push(`/MidManageInfo/1/${row.proId}`);
        },
        handlePublic(index,row){
            console.log(row.proId);
            var mystate=row.protime==="中期"?"结题":"中期";
            let params = {
                proId:row.proId,
                state:mystate
            }
